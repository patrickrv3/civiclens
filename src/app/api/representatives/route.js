...
// Zip code prefix → state abbreviation (first 3 digits)
const ZIP_TO_STATE = {
...
// State abbreviation → full name mapping for filtering API results
const STATE_NAMES = {
...
export async function POST(request) {
    try {
        ...
        const stateAbbr = zipToState(zipCode);
        if (!stateAbbr) {
        ...
        // Fetch ALL current members by paginating through the API
        let allMembers = [];
        let offset = 0;
        const limit = 250;

        while (true) {
            const url = `https://api.congress.gov/v3/member?currentMember=true&limit=${limit}&offset=${offset}&api_key=${apiKey}&format=json`;
            const response = await fetch(url);
            if (!response.ok) {
                return NextResponse.json({ error: 'Failed to fetch representatives' }, { status: 502 });
            }
        ...
        // Filter to only members from the user's state
        const stateMembers = allMembers.filter(m => m.state === stateName);

        // Current year for term end calculation
        const currentYear = new Date().getFullYear();

        // Split into senators and house members, calculate term end
        const senators = [];
        const houseMembers = [];
        ...
        for (const m of stateMembers) {
            ...
            const startYear = latestTerm?.startYear || 0;

            // Calculate CURRENT term end (not from first start year)
            let termEnd = '';
            if (chamber === 'Senate') {
                // Senate terms are 6 years, ending in even years
                // Calculate next term end from current year
                const yearsSinceStart = currentYear - startYear;
                const termsCompleted = Math.floor(yearsSinceStart / 6);
                termEnd = `${startYear + (termsCompleted + 1) * 6}`;
            } else if (chamber === 'House of Representatives') {
                // House terms are 2 years, ending in even years
                if (currentYear % 2 === 0) {
                    termEnd = `${currentYear + 2}`;
                } else {
                    termEnd = `${currentYear + 1}`;
                }
            }
            ...
            const member = {
                ...
                startYear,
                termEnd,
                url: `https://www.congress.gov/member/${m.name?.toLowerCase().replace(/[^a-z]/g, '-')}/${m.bioguideId}`,
            };
        ...
        // Sort: senators alphabetically, house members by district
        senators.sort((a, b) => a.name.localeCompare(b.name));
        houseMembers.sort((a, b) => (a.district || 0) - (b.district || 0));
